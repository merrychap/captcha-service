...
public class CaptchaResponseGenerator {
    ...
    public ModelAndView generateResponse(String page, Model model, String captcha) {
        for (String attr : params) {
            model.addAttribute(attr, "");
        }
        model.addAttribute(captchaAttribute, captcha);
        return new ModelAndView(page, (Map<String, ?>) model);
    }

    public ModelAndView generateResponse(String page, Model model, String captcha, boolean IsCorrect) {
    ...
    public ModelAndView generateResponse(String page, Model model, String captcha, boolean isCorrect, int errorCode) {
        if (isCorrect) {
            model.addAttribute(resultParam, correctAns);
            model.addAttribute(errorReason, "");
        } else {
